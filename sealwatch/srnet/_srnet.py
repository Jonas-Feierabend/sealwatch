...
class SRNet(nn.Module):
    def __init__(self, in_channels, num_classes, global_pooling='avg', activation=nn.ReLU(), norm_layer=nn.BatchNorm2d, norm_kwargs={}, **kwargs):
    ...
    @classmethod
    def pretrained_jin(cls, in_channels: int, *args, seed=None, **kw):

        # create model
        model = cls(in_channels=in_channels, *args, **kw)

        # download weights
        dde_checkpoint_url = 'https://github.com/DDELab/deepsteganalysis/releases/download/v0.1/jin_srnet.ckpt'
        state_dict = torch.hub.load_state_dict_from_url(dde_checkpoint_url)['state_dict']
        state_dict = {k.split('net.')[1]: v for k, v in state_dict.items()}

        # modify input
        if in_channels == 1:
            state_dict['block1.0.conv.weight'] = state_dict['block1.0.conv.weight'][:, 1:2]
        elif in_channels == 3:
            pass
        else:
            raise NotImplementedError(f'undefined initialization for {in_channels} channels')

        #
        model.load_state_dict(state_dict, strict=True)
        logging.info(f'using pretrained weights from {dde_checkpoint_url}')

        return model

    @classmethod
    def pretrained_benedikt(cls, weights_path, in_channels: int, *args, seed=None, **kw):

        # create model
        model = cls(in_channels=in_channels, *args, **kw)

        # load weights from Benedikt
        ...
        # initialize FC weights randomly
        generator = torch.Generator()
        if seed:
            generator.manual_seed(seed)
        state_dict['fc.weight'] = torch.empty_like(model.fc.weight.data).normal_(generator=generator)
        state_dict['fc.bias'] = torch.empty_like(model.fc.bias.data).normal_(generator=generator)

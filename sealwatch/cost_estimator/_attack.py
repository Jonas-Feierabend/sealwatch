    ...
    return float(
        np.sum(
            np.where(
                ...
                np.log(p_plus + 1e-15),
                np.where(
                    delta == -1, np.log(p_minus + 1e-15), np.log(p0 + 1e-15)
                ),
            )
        )
    )
...
def estimate_lambda(
    delta, rho_matrix, ternary=True, max_iter=70, damping_factor=0.5
):
    ...
    for _ in range(max_iter):
        ...
        # Multiplicative update, cf. Berger et al. (1996), instead of binary search (Filler & Fridrich, 2010)
        lambda_param *= (beta_theo / beta_obs) ** damping_factor

    return lambda_param


def estimate_lambda_directional(
    delta, rho_p1, rho_m1, max_iter=70, damping_factor=0.5
):
...
def attack(stego, cover, cover_spatial=None, qtable=None):
    ...
    if stego.ndim == 4:
        ...
        assert (
            cover_spatial is not None and qtable is not None
...
def attack_spatial(stego, cover):
    ...
    if np.array_equal(stego, cover):
        raise ValueError(
            "stego and cover are identical — did you pass the same image twice?"
        )
    ...
    for name, cost_func in cost_functions.items():
        ...
        if name == "lsbr":
            ...
            log_lik = (
                -np.inf
                if impossible
                else _log_likelihood(delta_arr, exponent, ternary=False)
            )
...
def attack_jpeg(stego_dct, cover_dct, cover_spatial, qtable):
    ...
    all_methods = {
        ...
        "juniward": (
            True,
            lambda: cl.juniward.compute_cost_adjusted(
                x0=cover_spatial, y0=cover_dct, qt=qtable
            ),
        ),
        "uerd": (
            True,
            lambda: cl.uerd.compute_cost_adjusted(y0=cover_dct, qt=qtable),
        ),
        "ebs": (
            True,
            lambda: cl.ebs.compute_cost_adjusted(y0=cover_dct, qt=qtable),
        ),
    }

    if np.array_equal(stego_dct, cover_dct):
        raise ValueError(
            "stego_dct and cover_dct are identical — did you pass the same image twice?"
        )
    ...
    for name, (ternary, cost_fn) in all_methods.items():
        ...
        # extra checks for better accuracy
        if _is_impossible(
            name, cover_dct, delta, wrong_direction, zero_changed
        ):
            results.append(
                {
                    "method_name": name,
